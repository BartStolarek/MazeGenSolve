...
import os
import sys
from agent_utility import Agent
...
def run_environment():
    ...
    if action != "wait":        		
       ...
       create_circle(agent.get_x(), agent.get_y(), dot_size, myCanvas,agent.get_colour(), agent.get_tag())
    if inter == iterations-1: 
        print("Max iterations of environment reached") 
        agent.print_model()
    if not quit:
       inter = inter + 1
       myCanvas.after(5, run_environment)

def on_closing():
    global quit
    if quit==False:
        quit = True
        myCanvas.after(50, on_closing)
    else:
        root.destroy()
...
def main():
    ...
    agent.print_details()
    Button(root, text="Quit", command = lambda: on_closing()).pack()
