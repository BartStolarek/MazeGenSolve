...
class Agent:
	...
	# Keep track of locations searched
        ...
        if self.check_model():
            return "stop"
        elif status == "odd" and self.model[model_x][model_y] == 0:
            self.p_moves =0
            self.model[model_x][model_y] = 1
            return "eat"
        elif status == 'even' and self.model[model_x][model_y] == 0 and self.p_moves > 10:
            self.p_moves =0
            self.model[model_x][model_y] = 1
            return "interact"
        elif self.model[model_x][model_y] == 0 and self.p_moves > 15:
            self.p_moves =0
            self.model[model_x][model_y] = 1
            return "move"
        elif self.p_moves > 20:
            self.p_moves =0
            return "move"
        else: 
            return "wait"
